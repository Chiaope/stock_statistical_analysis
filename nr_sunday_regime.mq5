...
input double   ADX_MinLevel   = 20.0;     // Minimum ADX to trade (Trend Strength)
...
string          mySymbol; 
int             atrHandle;
int             emaHandle;  // New EMA Handle
int             adxHandle;  // New ADX Handle
...
int OnInit()
  {
   ...
   // 2. Create EMA Handle
   emaHandle = iMA(mySymbol, PERIOD_D1, EMA_Period, 0, MODE_EMA, PRICE_CLOSE);
   
   // 3. Create ADX Handle
   adxHandle = iADX(mySymbol, PERIOD_D1, ADX_Period);
...
{
   if(SeriesInfoInteger(mySymbol, PERIOD_D1, SERIES_BARS_COUNT) < EMA_Period) return;
   ...
   if(r1 < r2 && r1 < r3 && r1 < r4)
   {
      ...
      // Copy last closed value (Index 1) to be safe/stable
      if(CopyBuffer(emaHandle, 0, 1, 1, emaVal) < 1) return;
      if(CopyBuffer(adxHandle, MAIN_LINE, 1, 1, adxMain) < 1) return;
      if(CopyBuffer(adxHandle, PLUSDI_LINE, 1, 1, adxPlus) < 1) return;
      if(CopyBuffer(adxHandle, MINUSDI_LINE, 1, 1, adxMinus) < 1) return;
      
      bool isUptrend = (lastClose > emaVal[0]);
      bool isStrong  = (adxMain[0] > ADX_MinLevel);
      bool isBullishADX = (adxPlus[0] > adxMinus[0]);

      if (!isUptrend) {
         Print("Filter Block: ADX below ", ADX_MinLevel, ". Market is Choppy. Skipping.");
         return;
      }
